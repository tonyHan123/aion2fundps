...
**Phase 1 시리즈 전체 완료** (2026-04-28). 데이터 레이어 100% 작동:
- 캡처 → 정렬 → 프레이밍 → opcode 분기 → 도메인 이벤트 → 통계 누적 → 신뢰도 추정 ✅
- 8개 opcode 핸들러 (DAMAGE, DOT, MOB_HP, COMBAT_BOUNDARY, SELF/OTHER_NICK, SUMMON, BUFF는 추후)
- 데이터 파일 통합 (몹 6,384 / 스킬 8,211, MIT)
- 타겟-공유 필터 (옆 사람 자동 제외, 검증된 26명 필터링)
- 보스 모드 자동 감지 (Entity 4800만 HP 추적 검증)
- 한글 스킬명 표시 (내려찍기, 파멸의 맹타 등)
- **신뢰도 배지** (다른 미터기에 없는 차별점)

다음: Phase 2 (영속화) 또는 Phase 3 (WPF UI). UI 갈 차례.
